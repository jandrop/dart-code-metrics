...
class UnusedFilesVisitor extends GeneralizingAstVisitor<void> {
  ...
  String? _getAbsolutePath(UriBasedDirective node) {
    if (node is ImportDirective) {
      return node.libraryImport?.importedLibrary?.firstFragment.source.fullName;
    }

    if (node is ExportDirective) {
      return node.libraryExport?.exportedLibrary?.firstFragment.source.fullName;
    }

    if (node is PartDirective) {
      final included = node.partInclude?.includedFragment;
      return included?.source.fullName;
    }
